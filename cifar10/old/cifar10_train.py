...
def cifar10_generate_data(N, corrupt_prob_list, noise_prob_list,
      x_original_file_list, y_file):

      (x_train_signal, y_train), (x_test_signal, y_test) = (
            tf.keras.datasets.cifar10.load_data())

      augmentation_one = "shift"
      #     print(x_train_signal.shape)
      #     print(np.unique(y_train[0:N]))
      print("Augmentation on test data: ", augmentation_one)
      # Preprocess the data (these are Numpy arrays)
      # x_train = x_train.reshape(60000, 28, 28, 1).astype('float32') / 255
      # x_test = x_test.reshape(10000, 28, 28, 1).astype('float32') / 255
      if augmentation_one == "rotation":
            datagen = tf.keras.preprocessing.image.ImageDataGenerator(rotation_range=75)
      if augmentation_one == "flip":
            datagen = tf.keras.preprocessing.image.ImageDataGenerator(horizontal_flip=True, vertical_flip=True)
      if augmentation_one == "shift":
            datagen = tf.keras.preprocessing.image.ImageDataGenerator(width_shift_range=0.2, height_shift_range=0.2) 

      for x_test, y_test_ in datagen.flow(x_test_signal, y_test, batch_size = x_test_signal.shape[0]):
            x_test_signal = x_test
            y_test = y_test_
            break
...
np.random.seed(0)
cifar10_generate_data(N=N, corrupt_prob_list=corrupt_prob_list,
